...
namespace FSH.BlazorChartjs.Util;
...
public static class ColorUtil
{
    ...
    /// <param name="g"></param>
    /// <param name="b"></param>
    public static string ColorHexString(byte r, byte g, byte b) => $"#{r:X2}{g:X2}{b:X2}";
    ...
    public static string ColorString(byte r, byte g, byte b, double alpha) => $"rgba({r}, {g}, {b}, {alpha.ToString(CultureInfo.InvariantCulture)})";
    ...
    /// </summary>
    public static string FromDrawingColor(Color color) =>
        color.A != byte.MaxValue
            ? ColorString(color.R, color.G, color.B, (double)color.A / byte.MaxValue)
            : ColorHexString(color.R, color.G, color.B);
}