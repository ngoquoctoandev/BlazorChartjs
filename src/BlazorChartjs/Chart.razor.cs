...
public partial class Chart : IDisposable
{
    ...
    private ChartJsInterop? JSModule { get; set; }
    ...
    {
        if (Config.Options is Options { OnMouseOutAsync: not null } options)
    ...
    public static async Task<ValueTask> OnClickAsync(DotNetObjectReference<IChartConfig> config, CallbackGenericContext ctx)
    {
        ...
        if (config.Value.Options is Options { OnClickAsync: not null } options)
    ...
    {
        if (config.Value.Options is Options { OnHoverAsync: not null } options)
    ...
    {
        if (config.Value.Options is Options { Plugins.Legend.OnClickAsync: not null } options)
